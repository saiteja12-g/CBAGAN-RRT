...
class RRT(object):
    ...
    def __init__(self, map):
        ...
        self.SIZE = 64 # Size of the world (100*100)

        self.q_init = [15.0, 53.0] # Start point
        self.q_final = [24.0, 18.0] # End point

        a, b = self.q_init
        c, d = self.q_final

        # print(map[a-1][b-1][0], map[a-1][b-1][1], map[a-1][b-1][2], map[c-1][d-1][0], map[c-1][d-1][1], map[c-1][d-1][2])

        # if map[a-1][b-1][0] == 0 or map[a-1][b-1][1] == 0 or map[a-1][b-1][2] == 0:
        #     print("Start node co-ordinates are outside the region of interest, please try again")
        #     quit()
        
        # if map[c-1][d-1][0] == 0 or map[c-1][d-1][1] == 0 or map[c-1][d-1][2] == 0:
        #     print("Goal node co-ordinates are outside the region of interest, please try again")
        #     quit()


        self.delta_q = 0.2 # Percentage of length to keep between q_rand and q_near when generating new point q_new
        self.dis_num = 64 # Discretization number of line collision checking
    ...
    def collision_check_line(self, q_start, q_end):
        ...
        for i in range(self.dis_num):
            x = int(q_start[0] + i * dx)
            y = int(q_start[1] + i * dy)
    ...
    def new_point_generate(self, q_near, q_rand, index_near):
        ...
        x = int(q_near[0] + (q_rand[0]-q_near[0]) * self.delta_q)
        y = int(q_near[1] + (q_rand[1]-q_near[1]) * self.delta_q)
        q_new = [x,y]
...
if __name__=="__main__":
    ...
    print("Time taken to reach goal node={:.3f}s ".format(time.time()-st))
    print("Number of nodes explored={} ".format(counter))
